...
while True:
	...
	if values['-EQ-'] == "":
	  ...
	  break
	EQ = values['-EQ-']
	if re.match('^(([\+\-]?\d*\/?\d*x?\^?[\+\-]?\d*\/?\d*))+$', EQ).bool == False:
	...
	def Differentiation():
		...
		for i in range(0, len(coefficient)):
			newcoeff.append(np.round((float(coefficient[i]) * float(exponent[i]), 4))) # multiplies the coefficient by its corrosponding exponent to get the new coefficient
			...
			if (float(exponent[i]) > 0):  # makes sure the expoent is not 0
				newexp.append(np.round((float(exponent[i]) - 1), 4)) # adds one to the exponent and sets it to the new exponent
